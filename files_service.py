...
def deploy_flask_app(app_name,env,project_path,current_path):
    try:
        ...
        os.system(f"sudo python3 -m venv {env}_{app_name}_venv")
        #os.system(f"source {env}_{app_name}_venv/bin/activate")
        os.system(f"sudo {env}_{app_name}_venv/bin/pip install -r requirements.txt")
        os.system(f"sudo {env}_{app_name}_venv/bin/pip install wheel")
        os.system(f"sudo {env}_{app_name}_venv/bin/pip install flask gunicorn")
...
def add_gunicorn_service(env,app_name,gunicorn_service):
    try:
        ...
        runbash_service.run_command("sudo systemctl daemon-reload")
        runbash_service.run_command(f"sudo systemctl start {env}_{app_name}")
        runbash_service.run_command(f"sudo systemctl enable {env}_{app_name}")
        return True  
    except OSError as os_error:
        print("Operating System Error:", os_error)
        return False
...
def add_nginx_config(service_name,app_name,nginx_config):
    try:
        ...
        runbash_service.run_command(f"sudo ln -s /etc/nginx/sites-available/{service_name} /etc/nginx/sites-enabled")
        runbash_service.run_command("sudo nginx -t")

            # Step 10: Restart Nginx
        runbash_service.run_command("sudo systemctl restart nginx")
        return True
    except OSError as os_error:
        print("Operating System Error:", os_error)
        return False
